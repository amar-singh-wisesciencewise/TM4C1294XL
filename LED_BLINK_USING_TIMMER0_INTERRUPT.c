...
#define T0_IM (*(volatile unsigned int*)(0x40030018))/*Interrupt MAsk*/
...
/*NVIC Registers*/
#define SET_ENABLE0 (*(volatile unsigned int*)(0xe000e100)) /*set enable 0 register of NVIC: to enable NVIC for INT 19 i.e Timer 0 A*/


void some_delay(unsigned int);
...
int main(void)
{
    ...
    T0TA_IL = (0x2FFFFFF); /*step 5: Load The interval count*/
    /*step 6: INterrupt MASK*/
    T0_IM |= (0x1u << 0);

    variable = SET_ENABLE0;
...
*/
        some_delay(2000000);
        GPIO_N_DATA_BASE[LED_TWO] ^= LED_TWO; /*LED-TWO IS BEING BLINKED FROM MAIN AND LED-ONE FROM HANDLER*/
	/*CHANGE THE LOAD INTERVAL AND DELAY VALUE TO OBSERVE AND CONFIRM*/
    }
    return 0;
}
...
void Timer0_A_Handler(void){ /*REGISTER THIS AS HANDLER*/

    GPIO_N_DATA_BASE[LED_ONE] ^= LED_ONE;
    T0_IC |= (0x1u << 0);/*clearing the bit sit after counter runoff*/
